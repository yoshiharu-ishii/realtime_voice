...
**キーのローテーション手順**(注: 新キーを発行しても旧キーは削除するまで有効。また実行中のタスクは起動時に読んだキーをメモリに持ち続ける):

1. OpenAIダッシュボードで新キーを発行(旧キーはまだ消さない)
2. `backend/.env` と `infra/service/secrets.auto.tfvars` を新キーに更新
3. `terraform -chdir=infra/service apply`(SSMが新キーになる)
4. `aws ecs update-service --force-new-deployment ...`(タスクが新キーを読み直す)
5. 本番の動作確認**後に**、ダッシュボードで旧キーを無効化(Inactive)または削除
6. **旧キーの失効を実測確認する**: 無効化は通常API(/v1/models等)には即時反映されるが、
   **Realtime系(client_secrets、wss://…/v1/realtime)には15〜20分遅れて届く**(2026-07-17実測)。
   「Inactiveにしたのにまだ通話できる」のはこのラグ。401を確認してからローテーション完了とする
